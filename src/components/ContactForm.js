  ...
  const {form,errors,loading,response,handleChange,handleBlur,handleSubmit} = useForm(initialForm,validationsForm);
    ...
    <div>
        <form onSubmit={handleSubmit}>
            ...
            <textarea name="comments" cols="50" rows="5" placeholder='Escribe aqui tus comentarios' onBlur={handleBlur} onChange={handleChange} value={form.comments} required> </textarea>
            {errors.comments && <p style={styles}>{errors.comments}</p>}
            <input type="submit" value="Enviar"/>
         </form>
         {loading && "Cargando..."}
         {response && "Se envio el formulario"}
    </div>
  )
}
